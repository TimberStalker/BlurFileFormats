...
public class EndiannessAwareBinaryReader : BinaryReader
{
    ...
    public Half ReadHalf(Endianness endianness) => BitConverter.ToHalf(ReadForEndianness(sizeof(float)/2, endianness));
    public float ReadSingle(Endianness endianness) => BitConverter.ToSingle(ReadForEndianness(sizeof(float), endianness));
